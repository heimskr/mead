...
namespace mead {
	Function::Function(std::weak_ptr<Program> program, std::string name, std::shared_ptr<Type> return_type, std::vector<std::shared_ptr<Type>> argument_types):
	Symbol(std::move(name)), weakProgram(std::move(program)), returnType(std::move(return_type)), argumentTypes(std::move(argument_types)), scope(weakProgram) {
		initBlocks();
	}
